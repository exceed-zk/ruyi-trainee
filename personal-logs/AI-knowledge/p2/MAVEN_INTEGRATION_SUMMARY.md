...
### 插件侧（生成 p2 仓库与 Feature）
- 父聚合 POM：`plugins/ruyisdk-eclipse-plugins/pom.xml`
- Feature（聚合全部插件）：
  - `plugins/ruyisdk-eclipse-plugins/features/org.ruyisdk.feature/feature.xml`
  - `plugins/ruyisdk-eclipse-plugins/features/org.ruyisdk.feature/pom.xml`
- p2 更新站点：
  - `plugins/ruyisdk-eclipse-plugins/repository/category.xml`
  - `plugins/ruyisdk-eclipse-plugins/repository/pom.xml`
  - 构建产物：`plugins/ruyisdk-eclipse-plugins/repository/target/repository/`
- 各插件模块 POM（示例）：
  - `plugins/ruyisdk-eclipse-plugins/plugins/org.ruyisdk.core/pom.xml`
  - `plugins/ruyisdk-eclipse-plugins/plugins/org.ruyisdk.devices/pom.xml`
  - `plugins/ruyisdk-eclipse-plugins/plugins/org.ruyisdk.intro/pom.xml`
  - `plugins/ruyisdk-eclipse-plugins/plugins/org.ruyisdk.news/pom.xml`
  - `plugins/ruyisdk-eclipse-plugins/plugins/org.ruyisdk.packages/pom.xml`
  - `plugins/ruyisdk-eclipse-plugins/plugins/org.ruyisdk.projectcreator/pom.xml`
  - `plugins/ruyisdk-eclipse-plugins/plugins/org.ruyisdk.ruyi/pom.xml`
  - `plugins/ruyisdk-eclipse-plugins/plugins/org.ruyisdk.ui/pom.xml`

### packages 侧（消费 p2 仓库并打 IDE）
- EPP 父 POM（注册 RuyiSDK 插件仓库、开启产品物化）：
  - `package/ruyisdk-eclipse-packages/releng/org.eclipse.epp.config/parent/pom.xml`
  - 关键点：
    - 属性：`ruyisdk.plugins.repository`
    - 仓库：`<repository id="ruyisdk-plugins" layout="p2" url="${ruyisdk.plugins.repository}"/>`
    - Profile：`epp.materialize-products`
- 顶层构建入口（选择参与构建的模块集）：
  - `package/ruyisdk-eclipse-packages/pom.xml`
  - 关键 profiles：`epp.p2.common`、`epp.p2.embedcpp`、`epp.product.embedcpp`
- 产品定义（将 `org.ruyisdk.feature` 放入最终 IDE）：
  - `package/ruyisdk-eclipse-packages/packages/org.eclipse.epp.package.embedcpp.product/epp.product`
...
# 2. 构建包含插件的 RuyiSDK IDE（显式开启相关 profiles）
...
## 集成原理
- plugins 侧：给每个插件/feature/repository 添加 Tycho POM，使其可被 Tycho 构建为带 p2 元数据的“可安装单元（IU）”与更新站点（repository/target/repository）。
- packages 侧：不通过 Maven 依赖坐标来“引用插件”，而是通过在父 POM注册一个 p2 仓库（`ruyisdk-plugins`）并在产品定义（`epp.product`）中包含 `org.ruyisdk.feature` 的方式来消费这些 IU。
- 构建链路：先构建 plugins 产出 p2 仓库 → EPP 在打包 IDE 时从该 p2 仓库解析并拉取 `org.ruyisdk.feature` → 将其连同依赖一起物化进最终 IDE。

## 参数说明（Profiles 与属性）
- `-Pepp.p2.common`：构建公共基础 p2 模块（公共包与 feature）。
- `-Pepp.p2.embedcpp`：构建 Embedded C/C++ 套件的 p2 模块与 feature。
- `-Pepp.product.embedcpp`：构建 Embedded C/C++ 产品仓库与产品定义。
- `-Pepp.materialize-products`：执行产品物化与归档（生成最终可分发压缩包）。
- `-Druyisdk.plugins.repository=...`：指定 RuyiSDK 自有插件的 p2 仓库地址（本地或远程），让 EPP 在构建时集成 `org.ruyisdk.feature`。Windows 本地路径必须为 `file:///D:/...`（三斜杠）。
```
...
| Unknown lifecycle phase ".package.embedcpp" | 使用了不存在的 profile | 改用 `-Pepp.p2.common -Pepp.p2.embedcpp -Pepp.product.embedcpp -Pepp.materialize-products` |
| URI has an authority component | Windows 本地 p2 URL 书写错误 | 使用 `file:///D:/...`（三斜杠），不要 `file://D:/...` |
